...
impl MyBalanceManager{
    ...
    pub fn run_balance_manager(&mut self){
        eprintln!("[balance manager ] Started (crossbeam batched mode) on core 6 first manager ");
        let mut count = 0u64;
...
//
    ...
    pub fn add_throughput_test_users(&mut self) {
        ...
        // Add user 10 (buyer)
        self.state.user_id_to_index.insert(10, 1);
        self.state.balances[1].user_id.store(10, Ordering::Relaxed);
        self.state.balances[1].available_balance.store(HIGH_BALANCE, Ordering::Relaxed);
        
        // Add user 20 (seller)
        self.state.user_id_to_index.insert(20, 2);
        self.state.balances[2].user_id.store(20, Ordering::Relaxed);
        self.state.balances[2].available_balance.store(HIGH_BALANCE, Ordering::Relaxed);
        
        // Give seller holdings for symbol 0
        for symbol in 0..MAX_SYMBOLS {
            self.state.holdings[2].available_holdings[symbol].store(HIGH_HOLDINGS, Ordering::Relaxed);
        }
        
        // ✅ VERIFY INITIALIZATION
        let user10_bal = self.state.balances[1].available_balance.load(Ordering::Relaxed);
        let user20_bal = self.state.balances[2].available_balance.load(Ordering::Relaxed);
        let user20_holdings = self.state.holdings[2].available_holdings[0].load(Ordering::Relaxed);
