...
use crate::orderbook::order::OrderToBeCanceled;
...
#[derive(Debug)]
pub struct CancelOrderQueue {
    mmap: MmapMut,
    header_ptr: *mut QueueHeader, // Cached pointer
    orders_ptr: *mut OrderToBeCanceled,       // Cached orders pointer
}

impl CancelOrderQueue {
    pub fn create<P: AsRef<Path>>(path: P) -> Result<Self, QueueError> {
        ...
        let orders_ptr = unsafe {
            mmap.as_mut_ptr().add(HEADER_SIZE) as *mut OrderToBeCanceled
        };
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self, QueueError> {
        ...
        // Cache both pointers
        let header_ptr = { mmap.as_mut_ptr() as *mut QueueHeader };
        let orders_ptr = unsafe { mmap.as_mut_ptr().add(HEADER_SIZE) as *mut OrderToBeCanceled };
    ...
    #[inline(always)]
    fn get_order(&self, pos: usize) -> OrderToBeCanceled {
        unsafe { *self.orders_ptr.add(pos) }
    }

    /// Set order at position - ZERO COST pointer arithmetic
    #[inline(always)]
    fn set_order(&self, pos: usize, order: OrderToBeCanceled) {
        unsafe {
    ...
    #[inline]
    pub fn dequeue(&mut self) -> Result<Option<OrderToBeCanceled>, QueueError> {
    ...
    pub fn enqueue(&mut self, order: OrderToBeCanceled) -> Result<(), QueueError> {
        let header = self.header_mut();
    ...
    pub fn dequeue_spin(&mut self, max_spins: usize) -> Result<Option<OrderToBeCanceled>, QueueError> {
...
impl Drop for CancelOrderQueue {
    fn drop(&mut self) {
...
unsafe impl Send for CancelOrderQueue {}
